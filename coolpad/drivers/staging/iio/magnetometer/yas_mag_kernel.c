...
#endif
	struct yas_mag_platform_data *pdata;
};
...
#endif
/*wujinrong add start*/
static int yas_mag_parse_dt(struct device *dev,
		struct yas_mag_platform_data *pdata)
{
	int rc;
	u32 temp_val;
	struct device_node *np = dev->of_node;
	rc = of_property_read_u32(np, "yas,mag_position", &temp_val);
	if (rc && (rc != -EINVAL)) {
		dev_err(dev, "Unable to read mag_position");
		return rc;
	} else
...
static int yas_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
	...
	int ret, i;
	struct yas_mag_platform_data *pdata = NULL;
	this_client = i2c;
	...
	if (i2c->dev.of_node) {
		pdata = devm_kzalloc(&i2c->dev,
			sizeof(struct yas_mag_platform_data), GFP_KERNEL);
		if (NULL == pdata) {
			dev_err(&i2c->dev, "failed to allocate memory\n");
			goto error_unregister_iio;
		}
		ret = yas_mag_parse_dt(&i2c->dev, pdata);
		if (ret) {
...
/*wujinrong add start*/
#ifdef CONFIG_OF
static const struct of_device_id yas_mag_of_match[] = {
	{.compatible = "yas_yas_magnetometer",},
	{ }
};
MODULE_DEVICE_TABLE(of, yas_mag_of_match);
#else
#define yas_mag_of_match NULL
#endif
/*wujinrong add end*/
...
static struct i2c_driver yas_driver = {
	.driver = {
		...
		#ifdef CONFIG_OF
		.of_match_table = yas_mag_of_match,
		#endif
		/*wujinrong add end*/
	},
	.probe		= yas_probe,
