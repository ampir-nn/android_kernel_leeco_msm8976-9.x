...
// initialize_vdm the VDM Manager. returns 0 on success.
int initialize_vdm_with_def (VdmManager* vdmm, VendorDefinition vdef) {
//	int i;
...
// initialize_vdm the VDM Manager (no definition/configuration object necessary). returns 0 on success.
int initialize_vdm (VdmManager* vdmm) {
	int i;
...
int discoverIdentities (VdmManager* vdmm, SopType sop) {
	...
	// TODO: validate sop is SOP or SOP'
...
int discoverModes(VdmManager* vdmm, SopType sop, UINT16 svid) {
	StructuredVdmHeader svdmh;
	unsigned int length = 1;
	UINT32 arr[length];
...
int processVdmMessage	(VdmManager* vdmm, SopType sop, UINT32* arr_in, unsigned int length_in)
{
...
//	UINT16* my_svids;
//	UINT32 (*my_modes)[MAX_MODES_PER_SVID];
	UINT16 svid_in ; //= svdmh_in.svid;		// grab the SVID
...
int enterMode			(VdmManager* vdmm, SopType sop, UINT16 svid, unsigned int mode_index) {
	StructuredVdmHeader svdmh;
	unsigned int length = 1;
	UINT32 arr[length];
...
int exitMode			(VdmManager* vdmm, SopType sop, UINT16 svid, unsigned int mode_index) {
	// TODO
