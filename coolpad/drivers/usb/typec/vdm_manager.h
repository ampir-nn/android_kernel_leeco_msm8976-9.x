 ...
 */
	...
	typedef struct {
		...
		UINT16				sop_svids[MAX_NUM_SVIDS_PER_SOP];
		unsigned int		num_sop_svids;

		UINT16				sop1_svids[MAX_NUM_SVIDS_PER_SOP];
		unsigned int		num_sop1_svids;

		UINT16				sop2_svids[MAX_NUM_SVIDS_PER_SOP];
		...
		UINT32				sop_modes [MAX_NUM_SVIDS_PER_SOP][MAX_MODES_PER_SVID];
		unsigned int		num_sop_modes_per_svid[MAX_NUM_SVIDS_PER_SOP];

		UINT32				sop1_modes[MAX_NUM_SVIDS_PER_SOP][MAX_MODES_PER_SVID];
		unsigned int		num_sop1_modes_per_svid[MAX_NUM_SVIDS_PER_SOP];

		UINT32				sop2_modes[MAX_NUM_SVIDS_PER_SOP][MAX_MODES_PER_SVID];
 ...
 */
	...
	int initialize_vdm(VdmManager* vdmm);								// initializes members without needing a definition
 ...
 */
	...
	int discoverModes		(VdmManager* vdmm, SopType sop, UINT16 svid);							// Discovers Modes available for each SVID
	int sendAttention		(VdmManager* vdmm, SopType sop);										// send attention command to specified sop
	int processVdmMessage	(VdmManager* vdmm, SopType sop, UINT32* arr, unsigned int length);		// function to call when we receive VDM messages
	int enterMode			(VdmManager* vdmm, SopType sop, UINT16 svid, unsigned int mode_index);	// enter mode specified by SVID and mode index
	int exitMode			(VdmManager* vdmm, SopType sop, UINT16 svid, unsigned int mode_index);	// exit mode specified by SVID and mode index
