...
static void red_led_blink(struct aw_leds_data *aw_data)
{
	...
	ret = aw_leds_i2c_write_reg(aw_data->pdata->AW_RED_T0,Rise_time<<4 | Hold_time);//ÁÁ¶È±£³Ö0.26s
	if (ret) {
		printk(KERN_ERR "%s: write_reg 0x%x failed\n", __func__, aw_data->pdata->AW_RED_T0);
		return;
	}

	ret = aw_leds_i2c_write_reg(aw_data->pdata->AW_RED_T1,Fall_time<<4 | Off_time);//µÆÃð2.08s
	if (ret) {
		printk(KERN_ERR "%s: write_reg 0x%x failed\n", __func__, aw_data->pdata->AW_RED_T1);
		return;
	}

	ret = aw_leds_i2c_write_reg(aw_data->pdata->AW_RED_T2,Delay_time);//ÑÓ³Ù0.26s¡£ÖÜÆÚ2.86s
	if (ret) {
...
static void red_led_off(struct aw_leds_data *aw_data)
{
	...
	ret = aw_leds_i2c_write_reg(AW_LEDE,aw_data->pdata->aw_red_off);//Ö»¹Ø±ÕºìÉ«µÆ
	if (ret) {
		printk(KERN_ERR "%s: write_reg 0x%x failed\n", __func__, AW_LEDE);
		return;
	}
...
static void green_led_blink(struct aw_leds_data *aw_data)
{
	...
	ret = aw_leds_i2c_write_reg(aw_data->pdata->AW_GREEN_T0,Rise_time<<4 | Hold_time);//ÁÁ¶È±£³Ö0.26s
	if (ret) {
		printk(KERN_ERR "%s: write_reg 0x%x failed\n", __func__, aw_data->pdata->AW_GREEN_T0);
		return;
	}

	ret = aw_leds_i2c_write_reg(aw_data->pdata->AW_GREEN_T1,Fall_time<<4 | Off_time);//µÆÃð2.08s
	if (ret) {
		printk(KERN_ERR "%s: write_reg 0x%x failed\n", __func__, aw_data->pdata->AW_GREEN_T1);
		return;
	}

	ret = aw_leds_i2c_write_reg(aw_data->pdata->AW_GREEN_T2,Delay_time);//ÑÓ³Ù0.26s¡£ÖÜÆÚ2.86s
	if (ret) {
...
static void green_led_off(struct aw_leds_data *aw_data)
{
	...
	ret = aw_leds_i2c_write_reg(AW_LEDE,aw_data->pdata->aw_green_off);//Ö»¹Ø±ÕÂÌÉ«µÆ
	if (ret) {
		printk(KERN_ERR "%s: write_reg 0x%x failed\n", __func__, AW_LEDE);
		return;
	}
...
static void blue_led_blink(struct aw_leds_data *aw_data)
{
	...
	ret = aw_leds_i2c_write_reg(aw_data->pdata->AW_BLUE_T0,Rise_time<<4 | Hold_time);//ÁÁ¶È±£³Ö0.26s
	if (ret) {
		printk(KERN_ERR "%s: write_reg 0x%x failed\n", __func__, aw_data->pdata->AW_BLUE_T0);
		return;
	}

	ret = aw_leds_i2c_write_reg(aw_data->pdata->AW_BLUE_T1,Fall_time<<4 | Off_time);//µÆÃð2.08s
	if (ret) {
		printk(KERN_ERR "%s: write_reg 0x%x failed\n", __func__, aw_data->pdata->AW_BLUE_T1);
		return;
	}

	ret = aw_leds_i2c_write_reg(aw_data->pdata->AW_BLUE_T2,Delay_time);//ÑÓ³Ù0.26s¡£ÖÜÆÚ2.86s
	if (ret) {
...
static void blue_led_off(struct aw_leds_data *aw_data)
{
	...
	ret = aw_leds_i2c_write_reg(AW_LEDE,aw_data->pdata->aw_blue_off);//Ö»¹Ø±ÕÀ¶É«µÆ
	if (ret) {
		printk(KERN_ERR "%s: write_reg 0x%x failed\n", __func__, AW_LEDE);
		return;
	}
...
static void red_led_on(struct aw_leds_data *aw_data)
{
	...
	ret = aw_leds_i2c_write_reg(AW_GCR,0x01);//´ò¿ªÈ«¾Ö¿ØÖÆ¼Ä´æÆ÷£¬Ê¹ÄÜLED×ÓÄ£¿é
	if (ret) {
		printk(KERN_ERR "%s: write_reg 0x%x failed\n", __func__, AW_GCR);
		return;
	}

	ret = aw_leds_i2c_write_reg(AW_LEDE,aw_data->pdata->aw_red_on);//ºìµÆ
	if (ret) {
		printk(KERN_ERR "%s: write_reg 0x%x failed\n", __func__, AW_LEDE);
		return;
	}
	...
	if(brightness){
		...
		switch(color) {
			case LED_COLOR_RED:
				...
				break;
			case LED_COLOR_GREEN:
				YL_DEBUG("*****GREEN  CLOSED*****\n");
				aw_data->led_flag[3]=0;
				green_led_off(aw_data);
				break;
			case LED_COLOR_BLUE:
				YL_DEBUG("*****BLUE CLOSED*****\n");
...
static int aw_led_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	ret = aw_leds_i2c_write_reg(AW_GCR,0x00);//¹Ø±ÕËùÓÐLED×ÓÄ£¿é,µÍ¹¦ºÄÄ£Ê½
	if(ret)
