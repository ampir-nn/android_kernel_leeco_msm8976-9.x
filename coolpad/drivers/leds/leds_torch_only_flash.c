...
#define LED_GPIO_FLASH_DRIVER_NAME	"rohm_flash,torch_only"
#define LED_TRIGGER_DEFAULT		"none"

struct led_torch_only_flash_data {
	int flash_en;
	int brightness;
...
static struct of_device_id led_torch_only_flash_of_match[] = {
	{.compatible = LED_GPIO_FLASH_DRIVER_NAME,},
	{},
};

static void led_torch_only_brightness_set(struct led_classdev *led_cdev,
				    enum led_brightness value)
{
	int rc = 0;
	struct led_torch_only_flash_data *flash_led =
	    container_of(led_cdev, struct led_torch_only_flash_data, cdev);
	...
	pr_err("CHYL E %s brightness=%d flash_en=%d \n",
		__func__, brightness, flash_en);
...
static enum led_brightness led_torch_only_brightness_get(struct led_classdev
						   *led_cdev)
{
	struct led_torch_only_flash_data *flash_led =
	    container_of(led_cdev, struct led_torch_only_flash_data, cdev);
...
int led_torch_only_flash_probe(struct platform_device *pdev)
{
	int rc = 0;
	const char *temp_str;
	struct led_torch_only_flash_data *flash_led = NULL;
	struct device_node *node = pdev->dev.of_node;
	flash_led = devm_kzalloc(&pdev->dev,
		sizeof(struct led_torch_only_flash_data), GFP_KERNEL);
	if (flash_led == NULL) {
	...
	flash_led->cdev.brightness_set = led_torch_only_brightness_set;
	flash_led->cdev.brightness_get = led_torch_only_brightness_get;
...
int led_torch_only_flash_remove(struct platform_device *pdev)
{
	struct led_torch_only_flash_data *flash_led =
	    (struct led_torch_only_flash_data *)platform_get_drvdata(pdev);
...
static struct platform_driver led_torch_only_flash_driver = {
	.probe = led_torch_only_flash_probe,
	.remove = led_torch_only_flash_remove,
	.driver = {
		   .name = LED_GPIO_FLASH_DRIVER_NAME,
		   .owner = THIS_MODULE,
		   .of_match_table = led_torch_only_flash_of_match,
		   }
};

static int __init led_torch_only_flash_init(void)
{
	return platform_driver_register(&led_torch_only_flash_driver);
}

static void __exit led_torch_only_flash_exit(void)
{
	return platform_driver_unregister(&led_torch_only_flash_driver);
}

late_initcall(led_torch_only_flash_init);
module_exit(led_torch_only_flash_exit);

MODULE_DESCRIPTION("torch_only LEDs driver");
MODULE_LICENSE("GPL v2");
MODULE_ALIAS("leds:leds_torch_only_flash");
