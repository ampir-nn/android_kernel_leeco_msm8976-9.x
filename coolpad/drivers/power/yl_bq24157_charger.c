...
struct bq24157_chip {
	...
	int         chg_vol_max;
	int         chg_curr_max;
	/*shiyan added for limit current*/
	int         save_chg_curr_now;
	/*shiyan added end*/
	int         chg_curr_now;
	int         iterm_ma;
	int         vsp_vol;
	...
	int        capa_design_mAH;
	/*shiyan added for limit current*/
	int        chg_current_limit_lcd_on;
	int        chg_current_not_limit_soc;
	/*shiyan added end*/
	int        rechg_capa;
	bool      vbus_present;
	...
	int        batt_temp_too_hot;
	int        batt_temp_cold;
	/*modify by sunxiaogang for codes platform design 2014.12.22 */
	int        batt_temp_too_cold;
	/*  */
	int        set_ivbus_max;
	...
	struct mutex			r_check_status_lock;
	...
	/*shiyan added for platform interface*/
...
/*shiyan added for limit current*/
extern int yl_get_lcd_status;  //defined in mdss_dsi.c
static int lcd_status_is_change = false;
static int call_status_is_change = false;
static int last_ivbus_max_save = -1;
static int last_lcd_status = false;
static int last_call_status = false;

extern int smartphone_calling_enable;
/*shiyan added end*/

/*shiyan added for recharge show 99% */
static bool batt_recharge_flag = false;
static int charge_last_state = 0;
/*shiyan added end*/
...
static int bq24157_set_chg_reg(struct bq24157_chip *chip)
{
	...
	/*shiyan add for charge reset*/
	/*set IC to reset mode*/
	rc = bq24157_masked_write(chip, BQ24157_IBAT, RESET_MASK, RESET_MASK);
	/*shiyan add end*/
...
static int check_bq24157_need_to_rechg(struct bq24157_chip *chip)
{
	...
	if ((chip->vbus_present) && (chip->rechg_capa >= chip->batt_capa)
...
static void bq24157_limit_charger_current(struct bq24157_chip *chip){
	mutex_lock(&chip->r_check_status_lock);
	if(chip->vbus_present){
		if(0 != chip->chg_current_limit_lcd_on){
			if(last_lcd_status^yl_get_lcd_status){ //lcd status is on now
				last_lcd_status = yl_get_lcd_status;
				lcd_status_is_change = true;
			}
			if (last_call_status ^ smartphone_calling_enable) {
				last_call_status = smartphone_calling_enable;
				call_status_is_change = true;
			}
		}
	}else{
		last_lcd_status = false;
		last_call_status = false;
	}
	mutex_unlock(&chip->r_check_status_lock);
}
...
static int bq24157_temp_appropriate_charging(struct bq24157_chip *chip)
{
	...
	/*shiyan add for power-off charging*/
	...
	/*shiyan add for power-off charging, the Service charger
	can not get a right value about the node :batterg_health
	in the first time,it will show normal charing then show
	too cold if the battery is really too cold.
	because yl_pm8916_vbus_probe OK is later than bq24157
	so we need read the temp again*/
	if (first_time) {
		first_time = false;
	...
	bq24157_limit_charger_current(chip);
	...
	if ((last_temp_sts != chip->batt_temp_status) || (last_volt_sts != batt_volt_low) ||lcd_status_is_change ||call_status_is_change) {
		last_temp_sts = chip->batt_temp_status;
		last_volt_sts = batt_volt_low;
		chip->batt_too_cold = false;
		chip->batt_too_hot = false;
		lcd_status_is_change = false;
		call_status_is_change = false;
		...
		if(chip->vbus_present){
			if(0 != chip->chg_current_limit_lcd_on){
				if(1 == yl_get_lcd_status){	 //lcd backlight is on
					if(0 != chip->chg_current_not_limit_soc){
						if(chip->batt_capa >= chip->chg_current_not_limit_soc)
							chip->chg_curr_max = chip->chg_current_limit_lcd_on;
						else
							chip->chg_curr_max = chip->save_chg_curr_now;
					}else{
						chip->chg_curr_max = chip->chg_current_limit_lcd_on;
					}
				}else{							//lcd backlight is off
					if (1 == smartphone_calling_enable)
						chip->chg_curr_max = chip->chg_current_limit_lcd_on;
					else
						chip->chg_curr_max = chip->save_chg_curr_now;
				}
		...
		pr_info("setting %d mA, %d, temp = %d, batt_temp_status = %d yl_get_lcd_status = %d chg_curr_max= %d smartphone_calling_enable= %d\n", chip->chg_curr_now,
				chg_current, chip->batt_temp, chip->batt_temp_status, yl_get_lcd_status, chip->chg_curr_max, smartphone_calling_enable);
...
#define ERR_BATT_TEMP        -22
/*shiyan add for charger error*/
#define BQ24157_STAT_FAULT      0x03
...
static void bq24157_update_heartbeat_work(struct work_struct *work)
{
	...
	charge_last_state = chip->charge_stat;
	chip->charge_stat = bq24157_get_stat(chip);
	if((BQ24157_STAT_DONE == charge_last_state) && (BQ24157_STAT_CHARGING == chip->charge_stat)) {
		batt_recharge_flag = true;
	}
	chip->batt_temp = get_yl_pm8916_batt_temp();
	chip->batt_volt = get_yl_pm8916_batt_mvol();

	if (ERR_BATT_TEMP == chip->batt_temp)
		return;

	/*shiyan for power off charging error*/
	if (BQ24157_STAT_FAULT == chip->charge_stat) {
	...
	pr_info("charge_stat=%d,vbus_present=%d,volt=%d,capa=%d,batt_temp*10=%d \n",
...
static int bq24157_get_prop_charge_type(struct bq24157_chip *chip)
{
	...
	if (chip->resume_completed)  {
		if (BQ24157_STAT_CHARGING == bq24157_get_stat(chip))
	ret.intval = POWER_SUPPLY_CHARGE_TYPE_FAST;
		else
			ret.intval =  POWER_SUPPLY_CHARGE_TYPE_NONE;
	} else{
		if (BQ24157_STAT_CHARGING == chip->charge_stat)
			ret.intval =  POWER_SUPPLY_CHARGE_TYPE_FAST;
		else
			ret.intval =  POWER_SUPPLY_CHARGE_TYPE_NONE;
	}
...
static int bq24157_get_prop_batt_capa(struct bq24157_chip *chip)
{
        ...
        if (chip->battery_psy != NULL) {
                ...
                /*shiyan modify for long time charging also show 99%*/
                if(1 == get_yl_pm8916_vbus_status()) {
                        if(((99 == ret.intval) && (BQ24157_STAT_DONE == chip->charge_stat)) || ((99 == ret.intval) && (true == batt_recharge_flag)))
                        {
                                ret.intval = 100;
                        }
                }
                else {
                        batt_recharge_flag = false;
                }
                /*add end*/
										...
										union power_supply_propval *val)
{
	...
	switch (prop) {
		...
		case POWER_SUPPLY_PROP_TEMP:
			...
			break;
		/*shiyan added for battery technology start*/
...
static void bq24157_external_power_changed(struct power_supply *psy)
{
	...
	pr_info("ivbus_max = %d\n", chip->set_ivbus_max);
	if (chip->set_ivbus_max ^ last_ivbus_max_save) {
		//pr_err("current_limit = %d last_ivbus_max_save= %d yl_get_lcd_status= %d \n", chip->set_ivbus_max,last_ivbus_max_save,yl_get_lcd_status);
		last_ivbus_max_save = chip->set_ivbus_max;
		rc = bq24157_set_ivbus_max(chip, chip->set_ivbus_max); //VBUS CURRENT
		if(0 == chip->set_ivbus_max){		//if the vbus is absent clear the value to default
			last_lcd_status = false;
			last_call_status = false;
			chip->chg_curr_max = chip->save_chg_curr_now;
		}else{
...
static int bq24157_parse_dt(struct bq24157_chip *chip)
{
	...
	if (rc < 0){
		...
		chip->save_chg_curr_now = chip->chg_curr_max;
	}
	...
	rc = of_property_read_u32(node, "yl,chg-current-limit-lcd-on", &chip->chg_current_limit_lcd_on);
	if (rc < 0){
		chip->chg_current_limit_lcd_on = 0;
		rc = 0;
	}

	rc = of_property_read_u32(node, "yl,chg-current-not-limit-soc", &chip->chg_current_not_limit_soc);
	if (rc < 0){
		dev_err(chip->dev, "Error reading chg_current_not_limit_soc\n");
		chip->chg_current_not_limit_soc = 0;
		rc = 0;
	}
	...
	rc = of_property_read_u32(node, "yl,recharge-capa", &chip->rechg_capa);
	if (rc < 0) {
		chip->rechg_capa = BATT_CAPA_RECHG;
		dev_err(chip->dev, "fail ===== chip->rechg_capa = %d \n", chip->rechg_capa);
		rc = 0;
	}
...
/*shiyan add for platform interface */
static enum power_supply_property bq24157_power_supply_props[] = {
	POWER_SUPPLY_PROP_VENDOR,
};

static int bq24157_power_supply_get_property(struct power_supply *psy,
				enum power_supply_property prop,
				union power_supply_propval *val)
{
	int ret = 0;
	struct bq24157_chip *chip = container_of(psy, struct bq24157_chip,
			charger);
...
static int bq24157_power_supply_set_property(struct power_supply *psy,
			enum power_supply_property prop,
			const union power_supply_propval *val)
{
	int ret = 0;
...
static int bq24157_power_supply_is_writeable(struct power_supply *psy,
			enum power_supply_property prop)
{
	int rc;
	struct bq24157_chip *chip = container_of(psy, struct bq24157_chip,
...
static void bq24157_psy_changed(struct power_supply *psy)
{
	struct bq24157_chip *chip = container_of(psy, struct bq24157_chip,
		charger);
	if (!chip) {
...
///bq24157 VBUS boost output in OTG mode. frankie. 2014.08.26 end.
	...
	/*shiyan add for charge reset*/
	static int bq24157_poweroff_notifier_call(struct notifier_block *this,
				unsigned long code, void *_cmd)
	{
		...
		if (this_chip->resume_completed == false)
			return ret;
			/* printk("bq24157 reset\n"); */
			bq24157_force_en_charging(this_chip, false);
			msleep(10);
			bq24157_set_chg_reg(this_chip);
			bq24157_force_en_charging(this_chip, true);

			return NOTIFY_DONE;
	}

	static struct notifier_block bq24157_poweroff_notifier = {
		.notifier_call = bq24157_poweroff_notifier_call,
	};
	/*shiyan added end*/

extern char charger_id;
...
static int bq24157_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	dev_info(&client->dev, "start bq24157_probe\n");
	...
	mutex_init(&chip->r_check_status_lock);
	...
	chip->charger.supplied_to = bq24157_batt_supplied_to;
	chip->charger.num_supplicants = ARRAY_SIZE(bq24157_batt_supplied_to);
	chip->charger.properties = bq24157_power_supply_props;
	chip->charger.num_properties = ARRAY_SIZE(bq24157_power_supply_props);
	chip->charger.get_property = bq24157_power_supply_get_property;
	chip->charger.set_property = bq24157_power_supply_set_property;
	chip->charger.property_is_writeable	=
	 bq24157_power_supply_is_writeable;
	chip->charger.external_power_changed = bq24157_psy_changed;
	rc = power_supply_register(chip->dev, &chip->charger);
	if (rc < 0) {
		dev_err(&client->dev,
			"Unable to register charger rc = %d\n", rc);
		goto fail_hw_init;
	}
	/*shiyan added end*/
...
/* bq24157 en pin initialize */
	...
	/*shiyan add for charge reset*/
	register_reboot_notifier(&bq24157_poweroff_notifier);
	/*shiyan add end*/
...
static int bq24157_remove(struct i2c_client *client)
{
	...
	mutex_destroy(&chip->r_check_status_lock);
	cancel_delayed_work_sync(&chip->update_heartbeat_work);
