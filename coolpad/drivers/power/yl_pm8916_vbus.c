...
/*shiyan add a default value*/
#define BATT_TEMP_DEFAULT      280   /* 28C */
#define BATT_TEMP_ERR_HIGH     790   /* 79C */
#define BATT_TEMP_ERR_LOW      -300  /* -30C */
/*shiyan add end*/
int get_yl_pm8916_batt_temp(void)
{
	...
	/*shiyan add a default value*/
	if ((BATT_TEMP_ERR_LOW >= results.physical) || (BATT_TEMP_ERR_HIGH <= results.physical))
		results.physical = BATT_TEMP_DEFAULT;
	/*shiyan add end*/
...
late_initcall(yl_pm8916_vbus_init);
