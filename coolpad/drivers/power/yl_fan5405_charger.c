...
static void fan5405_limit_charger_current(struct fan5405_chip *chip){
...
static int fan5405_temp_appropriate_charging(struct fan5405_chip *chip)
{
	...
	fan5405_limit_charger_current(chip);
...
static void fan5405_update_heartbeat_work(struct work_struct *work)
{
	...
	if((FAN5405_STAT_DONE == charge_last_state) && (FAN5405_STAT_CHARGING == chip->charge_stat)) {
		batt_recharge_flag = true;
		pr_err("==========start recharge======== \n");
	}
	chip->batt_temp = get_yl_pm8916_batt_temp();
	chip->batt_volt = get_yl_pm8916_batt_mvol();

	/*add begin by sunxiaogang@yulong.com for power off charging error*/
	if (ERR_BATT_TEMP == chip->batt_temp)
	    return;
	/*add end by sunxiaogang@yulong.com for power off charging error*/
...
static int fan5405_get_prop_batt_capa(struct fan5405_chip *chip)
{
	union power_supply_propval ret = {0,};
        ...
        if (chip->battery_psy != NULL) {
                ...
                if(1 == get_yl_pm8916_vbus_status()) {
                        if(((99 == ret.intval) && (FAN5405_STAT_DONE == chip->charge_stat)) || ((99 == ret.intval) && (true == batt_recharge_flag)))
                        {
                                ret.intval = 100;
                        }
                }
...
static enum power_supply_property fan5405_power_supply_props[] = {
	POWER_SUPPLY_PROP_VENDOR,
};

static int fan5405_power_supply_get_property(struct power_supply *psy,
				enum power_supply_property prop,
				union power_supply_propval *val)
{
	int ret = 0;
	struct fan5405_chip *chip = container_of(psy, struct fan5405_chip,
			charger);
	if (!chip) {
		/* LOGE("null pointer\n"); */
		ret = -1;
...
static int fan5405_power_supply_set_property(struct power_supply *psy,
				enum power_supply_property prop,
				const union power_supply_propval *val)
{
	int ret = 0;
...
static int fan5405_power_supply_is_writeable(struct power_supply *psy,
				enum power_supply_property prop)
{
	int rc;
	struct fan5405_chip *chip = container_of(psy, struct fan5405_chip,
...
static void fan5405_psy_changed(struct power_supply *psy)
{
	struct fan5405_chip *chip = container_of(psy, struct fan5405_chip,
			charger);
	if (!chip) {
...
///fan5405 VBUS boost output in OTG mode. frankie. 2014.08.26 start.
int fan5405_enable_otg_mode(void)
{
    int rc = 0;

    pr_info("fan5405 OTG VBUS enable\n");
...
int fan5405_disable_otg_mode(void)
{
    int rc = 0;

    pr_info("fan5405 OTG VBUS disable\n");
...
///fan5405 VBUS boost output in OTG mode. frankie. 2014.08.26 end.
...
static int fan5405_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	chip->charger.supplied_to = fan5405_batt_supplied_to;
	chip->charger.num_supplicants = ARRAY_SIZE(fan5405_batt_supplied_to);
	chip->charger.properties = fan5405_power_supply_props;
	chip->charger.num_properties = ARRAY_SIZE(fan5405_power_supply_props);
	chip->charger.get_property = fan5405_power_supply_get_property;
	chip->charger.set_property = fan5405_power_supply_set_property;
	chip->charger.property_is_writeable	=
		fan5405_power_supply_is_writeable;
	chip->charger.external_power_changed = fan5405_psy_changed;
