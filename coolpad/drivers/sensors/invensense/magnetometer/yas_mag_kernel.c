...
/*YuLong add start for position 20150129*/
#ifdef CONFIG_OF
#include <linux/of.h>
...
static int yas_parse_dt(struct device *dev)
{
	...
	unsigned int value;
        ...
        ret = of_property_read_u32(np, "yas,position", (unsigned int *)&value);
        if (ret) {
                dev_err(dev, "Looking up %s property in node %s failed",
                        "yas,position", np->full_name);
                return -ENODEV;
        }
	yas_position = value;
	printk("%s: yas_position = %d\n", __func__, yas_position);
...
/*YuLong add start for position 20150129*/
#ifdef CONFIG_OF
	ret = yas_parse_dt(&i2c->dev);
	if (ret) {
		dev_err(&i2c->adapter->dev, "yas_parse_dt() failed\n");
		goto error_ret;
	}
#endif
/*add end*/
