...
/*YuLong add start for accel calibration 20150129*/
#ifdef CONFIG_YL_ACCEL_CALIBRATION
#include <linux/sensors/sensparams.h>

#define MPU6XXX_CAL_CONVERT 16384
#define MPU6XXX_CAL_FUZZ 6000
#endif
/*add end*/

#ifdef CONFIG_INV_I2C_TIMEOUT_WAIT
int inv_resume = 1;
extern wait_queue_head_t wait_inv_resume;
#endif

#ifdef CONFIG_YL_SENSORS_DEBUG
	#define MPU_DEBUG(fmt, args...) pr_info(fmt, ##args)
#else
...
#include <linux/fb.h>
static int inv_fb_notifier_callback(struct notifier_block *self,
...
static int mpu_parse_dt(struct device *dev, struct mpu_platform_data *pdata)
{
	int i, ret;
	unsigned int value;
	int orientation[9];
	...
	//pdata->pm_suspend = NULL;
	//pdata->pm_resume = NULL;

	/* parse irq and request gpio */
        printk("%s(): np=%p\n",__func__,np);
	value = of_get_named_gpio_flags(np, "inv,int-gpio", 0, &flags);
	if (value < 0) {
		return -EINVAL;
	}
	gpio_request(value, "mpu-irq");
	gpio_direction_input(value);

	ret = of_property_read_u32(np, "inv,int-config", (unsigned int *)&value);
	if (ret) {
		dev_err(dev, "Looking up %s property in node %s failed",
			"inv,int_config", np->full_name);
		return -ENODEV;
	}
	pdata->int_config = value;
	ret = of_property_read_u32(np, "inv,level-shifter", (unsigned int *)&value);
	if (ret) {
		dev_err(dev, "Looking up %s property in node %s failed",
			"inv,level_shifter", np->full_name);
		return -ENODEV;
	}
	pdata->level_shifter = value;

	ret = of_property_read_u32_array(np, "inv,orientation", (unsigned int *)orientation, 9);
	if (ret) {
		dev_err(dev, "Looking up %s property in node %s failed",
			"inv,orientation", np->full_name);
		return -ENODEV;
	}
	for (i  = 0; i < 9; i++) {
...
static int set_power_itg(struct inv_mpu_iio_s *st, bool power_on)
{
	...
	if (power_on) {
		//msleep(POWER_UP_TIME);/*YuLong delete for test 20150123*/
...
/*YuLong add start for accel calibration 20150129*/
#ifdef CONFIG_YL_ACCEL_CALIBRATION
static int inv_calibration(struct inv_mpu_iio_s *st, int count)
{
	int i = 0;
	int result = 0;
	uint8_t buffer[8];
	...
	for (i = 0; i < count; i++) {
		...
		result = inv_i2c_read(st, REG_RAW_ACCEL, 6, &(buffer[0]));
		if (result < 0) {
			printk("%s: inv_i2c_read() failed\n", __func__);
			return result;
		}
		rawdata_x = (short)( buffer[1] | (buffer[0]<<8) );
		rawdata_y = (short)( buffer[3] | (buffer[2]<<8) );
		rawdata_z = (short)( buffer[5] | (buffer[4]<<8) );
		MPU_DEBUG("%s: rawdata_x = %d, rawdata_y = %d, rawdata_z = %d\n",
			__func__, rawdata_x, rawdata_y, rawdata_z);
	...
	if (((xavg > -MPU6XXX_CAL_FUZZ)&&(xavg < MPU6XXX_CAL_FUZZ))
		&&((yavg > -MPU6XXX_CAL_FUZZ)&&(yavg < MPU6XXX_CAL_FUZZ))
		&&((zavg > (MPU6XXX_CAL_CONVERT-MPU6XXX_CAL_FUZZ))&&
		(zavg < (MPU6XXX_CAL_CONVERT+MPU6XXX_CAL_FUZZ))))
	{
		st->accel_offset[0] = xavg;
		st->accel_offset[1] = yavg;
		st->accel_offset[2] = zavg - MPU6XXX_CAL_CONVERT;
		result = 0;
	} else if (((xavg > -MPU6XXX_CAL_FUZZ)&&(xavg < MPU6XXX_CAL_FUZZ))
		&&((yavg > -MPU6XXX_CAL_FUZZ)&&(yavg < MPU6XXX_CAL_FUZZ))
		&&((zavg > (-MPU6XXX_CAL_CONVERT-MPU6XXX_CAL_FUZZ))&&
		(zavg < (-MPU6XXX_CAL_CONVERT+MPU6XXX_CAL_FUZZ)))) {
		st->accel_offset[0] = xavg;
	...
	printk("%s: xavg = %d, yavg = %d, zavg = %d\n", __func__, xavg, yavg, zavg);
	return result;
}

static ssize_t inv_calibration_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	u8 d;
	int result = 0;
	...
	result = inv_i2c_read(st, REG_6500_LP_ACCEL_ODR, 1, &d);
	result = inv_i2c_single_write(st, REG_6500_LP_ACCEL_ODR, 8);/*62.5HZ*/

	//mutex_lock(&st->mutex);
	result = inv_calibration(st, 20);
	((struct acc_offset *)acc_cal_ptr)->x = st->accel_offset[0];
	((struct acc_offset *)acc_cal_ptr)->y = st->accel_offset[1];
	((struct acc_offset *)acc_cal_ptr)->z = st->accel_offset[2];
	((struct acc_offset *)acc_cal_ptr)->key = result ? 2 : 1;
	MPU_DEBUG("%s xoffset = %d\n",  __func__, ((struct acc_offset *)acc_cal_ptr)->x);
	MPU_DEBUG("%s yoffset = %d\n",  __func__, ((struct acc_offset *)acc_cal_ptr)->y);
	MPU_DEBUG("%s zoffset = %d\n",  __func__, ((struct acc_offset *)acc_cal_ptr)->z);
	MPU_DEBUG("%s CalStatus = %d\n",  __func__, ((struct acc_offset *)acc_cal_ptr)->key);
	sensparams_write_to_flash(SENSPARAMS_TYPE_ACC,
		(unsigned char*)acc_cal_ptr, sizeof(acc_cal_data));
	//mutex_unlock(&st->mutex);
...
static ssize_t inv_calibration_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct inv_mpu_iio_s *st = iio_priv(dev_get_drvdata(dev));
	struct acc_offset acc_cal_data;
	...
	sensparams_read_from_flash(SENSPARAMS_TYPE_ACC,
		(unsigned char*)acc_cal_ptr, sizeof(acc_cal_data));
	st->accel_offset[0] = ((struct acc_offset *)acc_cal_ptr)->x;
	st->accel_offset[1] = ((struct acc_offset *)acc_cal_ptr)->y;
	st->accel_offset[2] = ((struct acc_offset *)acc_cal_ptr)->z;
	printk("%s xoffset = %d\n",  __func__, ((struct acc_offset *)acc_cal_ptr)->x);
	printk("%s yoffset = %d\n",  __func__, ((struct acc_offset *)acc_cal_ptr)->y);
	printk("%s zoffset = %d\n",  __func__, ((struct acc_offset *)acc_cal_ptr)->z);
	printk("%s CalStatus = %d\n",  __func__, ((struct acc_offset *)acc_cal_ptr)->key);

	return sprintf(buf, "X%d Y%d Z%d K%d\n",
		st->accel_offset[0], st->accel_offset[1], st->accel_offset[2],
		((struct acc_offset *)acc_cal_ptr)->key);
}
#endif
/*add end*/
#endif
...
/*YuLong add start for accel calibration 20150129*/
#ifdef CONFIG_YL_ACCEL_CALIBRATION
static DEVICE_ATTR(accl_calibration, S_IRUGO| S_IWUGO, inv_calibration_show, inv_calibration_store);
#endif
/*add end*/
...
static struct attribute *inv_raw_i2c_mpu65xx_attributes[] = {
	...
	/*YuLong add start for accel calibration 20150129*/
	#ifdef CONFIG_YL_ACCEL_CALIBRATION
        &dev_attr_accl_calibration.attr,
	#endif
	/*add end*/
};
#endif
		...
		const struct i2c_device_id *id)
{
	...
	if (!strcmp(id->name, "itg3500"))
		...
		st->chip_type = INV_MPU6500;
	else if (!strcmp(id->name, "mpu688x"))
		st->chip_type = INV_MPU6500;
	...
	const struct i2c_device_id *id)
{
	...
	result = mpu_parse_dt(&client->dev, pdata);
	if (result) {
	...
	mutex_init(&st->mutex);
...
#if defined(CONFIG_FB)
        indio_dev->fb_notif.notifier_call = inv_fb_notifier_callback;
        result = fb_register_client(&indio_dev->fb_notif);
        if(result) {
                pr_err("Unable to register fb_notif\n");
                goto out_unreg_fb;
        }
#endif
        ...
        sensors_gyro_cdev.vendor = indio_dev->name;
        result = sensors_classdev_register(&client->dev, &sensors_gyro_cdev);
        if (result) {
                pr_err("%s: Unable to register to sensors class: %d\n",
                                __func__, result);
        }

        sensors_accelerometer_cdev.vendor = indio_dev->name;
        result = sensors_classdev_register(&client->dev, &sensors_accelerometer_cdev);
        if (result) {
                pr_err("%s: Unable to register to sensors class: %d\n",
                                __func__, result);
        }
...
#if defined(CONFIG_FB)
out_unreg_fb:
        fb_unregister_client(&indio_dev->fb_notif);
#endif
out_unreg_iio:
...
#ifdef CONFIG_INV_I2C_TIMEOUT_WAIT
	inv_resume = 1;
	wake_up_interruptible(&wait_inv_resume);
#endif
        ...
        unsigned long event, void *data)
{
        ...
        struct iio_dev *indio_dev =
                container_of(self, struct iio_dev, fb_notif);

        if (evdata && evdata->data && (event == FB_EVENT_BLANK) && indio_dev) {
                blank = evdata->data;
                if (*blank == FB_BLANK_UNBLANK)
                        inv_mpu_resume(&(indio_dev->dev));
                else if (*blank == FB_BLANK_POWERDOWN)
                        inv_mpu_suspend(&(indio_dev->dev));
        }
