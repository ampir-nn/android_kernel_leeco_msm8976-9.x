...
extern int inv_resume;
DECLARE_WAIT_QUEUE_HEAD(wait_inv_resume);
EXPORT_SYMBOL(wait_inv_resume);
#endif
/**
...
#ifdef CONFIG_INV_I2C_TIMEOUT_WAIT
	wait_event_interruptible(wait_inv_resume,inv_resume == 1);
#endif
	mutex_lock(&st->mutex);
	reg = &st->reg;
