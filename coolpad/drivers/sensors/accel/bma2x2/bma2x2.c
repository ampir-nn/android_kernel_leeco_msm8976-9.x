...
#include <linux/sensors/sensparams.h>//longjiang delete 20130924
	...
	if (enable) {
		if (pre_enable == 0) {
			...
			schedule_delayed_work(&bma2x2->work,
				3*msecs_to_jiffies(atomic_read(&bma2x2->delay)));
#endif
...
static DEVICE_ATTR(enable_int, S_IWUSR|S_IRGRP|S_IROTH,
...
static DEVICE_ATTR(softreset, S_IWUSR|S_IRGRP|S_IROTH,
...
static int bosch_accel_calibrate(int count)
{
	...
	y_cali = y_cali/count;
	z_cali = z_cali/count;
	BMA_DEBUG("%s: x_cali = %d, y_cali = %d, z_cali = %d\n",__func__,x_cali,y_cali,z_cali);

	if(((x_cali > -CAL_FUZZ)&&(x_cali < CAL_FUZZ)) && ((y_cali > -CAL_FUZZ)&&(y_cali < CAL_FUZZ)) && (((z_cali > (CAL_CONVERT - CAL_FUZZ))&&(z_cali < (CAL_CONVERT + CAL_FUZZ))) || ((z_cali > (-CAL_CONVERT - CAL_FUZZ))&&(z_cali < (-CAL_CONVERT + CAL_FUZZ)))))
	{
...
static long bosch_acc_ioctl(struct file *file,unsigned int cmd,unsigned long arg)
{
	...
	switch (cmd)
	{
		...
		case GSENSOR_IOCTL_APP_CALIBRATE:
			...
			BMA_DEBUG("%s write: x = %d\n",__func__,acc_cal_data.x);
			BMA_DEBUG("%s write: y = %d\n",__func__,acc_cal_data.y);
			BMA_DEBUG("%s write: z = %d\n",__func__,acc_cal_data.z);
			BMA_DEBUG("%s write: key = %d\n",__func__,acc_cal_data.key);

			if(copy_to_user(argp,&acc_cal_data,sizeof(acc_cal_data)))
				printk(KERN_ERR"%s: calibrate copy_to_user failed!\n",__func__);

			/* write to flash */
			//longjiang delete 20130924
			sensparams_write_to_flash(SENSPARAMS_TYPE_ACC,(unsigned char *)&acc_cal_data,sizeof(struct acc_offset));
			BMA_DEBUG("%s: write calibrate value to flash finish!\n",__func__);
			break;

		case GSENSOR_IOCTL_APP_OFFSET:
			/* read from flash */
			BMA_DEBUG("%s: command GSENSOR_IOCTL_APP_OFFSET!\n",__func__);
			sensparams_read_from_flash(SENSPARAMS_TYPE_ACC,(unsigned char *)&acc_cal_data, sizeof(struct acc_offset));
			...
			BMA_DEBUG("%s read: x offset = %d\n", __func__, acc_cal_data.x);
			BMA_DEBUG("%s read: y offset = %d\n", __func__, acc_cal_data.y);
			BMA_DEBUG("%s read: z offset = %d\n", __func__, acc_cal_data.z);
			BMA_DEBUG("%s read: calibrate status = %d\n", __func__, acc_cal_data.key);
		...
		const struct i2c_device_id *id)
{
	...
	#ifdef CONFIG_OF
	sensors_power_init(data,1);
	sensors_power_on(data,1);
	msleep(500);
	#endif

	/* read chip id */
	for (i = 0; i < 3; i++) {
		tempvalue = i2c_smbus_read_word_data(client,
			BMA2X2_CHIP_ID_REG);
		if (tempvalue < 0) {
			BMA_DEBUG("Read chip id times = %d\n", i);
			msleep(100);
		} else {
			break;
		}
...
static int bma2x2_suspend(struct i2c_client *client, pm_message_t mesg)
{
	struct bma2x2_data *data = i2c_get_clientdata(client);
	#ifdef SENSORS_I2C_RETRY_ERR
		pr_err("%s: i2c_retry_err = %d\n", __func__, i2c_retry_err);
		if (i2c_retry_err == 0) {
	#endif
			mutex_lock(&data->enable_mutex);
			if (atomic_read(&data->enable) == 1) {
