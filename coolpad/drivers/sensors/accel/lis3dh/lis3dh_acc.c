...
//zb add 20140117
struct lis3dh_acc_data *lis3dh_acc = NULL;
//zb end
...
struct sensor_regulator lis3dh_acc_vreg[] = {
	{NULL, "vdd_ana", 1700000, 3600000},
	{NULL, "vcc_i2c", 1700000, 3600000},
};
...
static int lis3dh_acc_config_regulator(struct lis3dh_acc_data *acc, bool on)
{
	...
	printk(KERN_INFO "YL:sensors:zbtest on = %d,acc->client->dev.platform_data =0x%p\n", on, acc->client->dev.platform_data);
	if (on) {
		for (i = 0; i < num_reg; i++) {
...
static int lis3dh_acc_device_power_on(struct lis3dh_acc_data *acc)
{
	...
	printk(KERN_INFO "YL:sersors:zbtest acc->pdata->gpio_int1 = %d,acc->pdata->gpio_int2 = %d\n", acc->pdata->gpio_int1, acc->pdata->gpio_int2);
					    ...
					    int *xyz)
{
	...
	xyz[0] = (((acc->pdata->negate_x) ? (-hw_d[acc->pdata->axis_map_x])
		  ...
		  acc->accel_cali[acc->pdata->axis_map_x]) / 4;
	xyz[1] = (((acc->pdata->negate_y) ? (-hw_d[acc->pdata->axis_map_y])
		   : (hw_d[acc->pdata->axis_map_y])) -
		  acc->accel_cali[acc->pdata->axis_map_y]) / 4;
	xyz[2] = (((acc->pdata->negate_z) ? (-hw_d[acc->pdata->axis_map_z])
		   : (hw_d[acc->pdata->axis_map_z])) -
		  acc->accel_cali[acc->pdata->axis_map_z]) / 4;
...
static void lis3dh_acc_input_work_func(struct work_struct *work)
{
	...
	acc->pdata->poll_interval = acc_delay_ms;
	schedule_delayed_work(&acc->input_work,
			      msecs_to_jiffies(acc->pdata->poll_interval));
	mutex_unlock(&acc->lock);
}
...
//longjiang add platform code start 20130520
static int lis3dh_acc_read_data(struct lis3dh_acc_data *acc, int *xyz)
{
	int err = -1;
	/* Data bytes from hardware xL, xH, yL, yH, zL, zH */
	u8 acc_data[6];
	...
	xyz[acc->pdata->axis_map_x] =
	    ((acc->pdata->negate_x) ? (-hw_d[acc->pdata->axis_map_x])
	     : (hw_d[acc->pdata->axis_map_x]));
	xyz[acc->pdata->axis_map_y] =
	    ((acc->pdata->negate_y) ? (-hw_d[acc->pdata->axis_map_y])
	     : (hw_d[acc->pdata->axis_map_y]));
	xyz[acc->pdata->axis_map_z] =
	    ((acc->pdata->negate_z) ? (-hw_d[acc->pdata->axis_map_z])
	     : (hw_d[acc->pdata->axis_map_z]));
...
static int lis3dh_acc_Calibration(int count)
{
	...
	struct lis3dh_acc_data *acc = lis3dh_acc;
	int xyz[3] = { 0 };

	memset(&lis3dh_acc->accel_cali, 0, sizeof(lis3dh_acc->accel_cali));
	...
	if (((xavg > -CAL_FUZZ) && (xavg < CAL_FUZZ))
	    ...
	    && ((zavg > (CAL_CONVERT - CAL_FUZZ))
		&& (zavg < (CAL_CONVERT + CAL_FUZZ)))) {
		lis3dh_acc->accel_cali[acc->pdata->axis_map_x] = xavg;
		lis3dh_acc->accel_cali[acc->pdata->axis_map_y] = yavg;
		lis3dh_acc->accel_cali[acc->pdata->axis_map_z] =
		    zavg - CAL_CONVERT;
		printk(KERN_INFO "%s(9.8): success!\n", __func__);
		ret = 0;
	} else if (((xavg > -CAL_FUZZ) && (xavg < CAL_FUZZ))
		   && ((yavg > -CAL_FUZZ) && (yavg < CAL_FUZZ))
		   && ((zavg > (-CAL_CONVERT - CAL_FUZZ))
		       && (zavg < (-CAL_CONVERT + CAL_FUZZ)))){
		lis3dh_acc->accel_cali[acc->pdata->axis_map_x] = xavg;
...
static int lis3dh_acc_input_init(struct lis3dh_acc_data *acc)
{
	...
	acc->input_dev->name = GSENSOR_INPUT_NAME;
...
#ifdef CONFIG_OF
static int lis3dh_accel_parse_dt(struct device *dev,
				 struct lis3dh_acc_platform_data *pdata)
{
	int rc;
	...
	//printk("YL:sensor-zbtest rc=%d,temp_val=%d,on line %d\n",rc,temp_val,__LINE__);
	if (rc && (rc != -EINVAL)) {
	...
	if (rc && (rc != -EINVAL)) {
		...
		pdata->gpio_int2 = -EINVAL;

	dev_info(dev, "poll_interval = %d, min_interval = %d\n",
		 pdata->poll_interval, pdata->min_interval);
	dev_info(dev, "accel_g_range = %u\n", pdata->g_range);
//      dev_info(dev, "accel_res = %d, accel_direction = %u\n", pdata->accel_res, pdata->accel_direction);
...
{
	printk(KERN_INFO "BMA SENSOR's device node is opened in func: %s\n",
	       __func__);
	return 0;
}
...
{
	printk(KERN_INFO "BMA SENSOR's device node is closed in func: %s\n",
	       __func__);
	return 0;
}
...
static long lis3dh_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if (NULL == acc || NULL == argp)
		return -EINVAL;

	printk(KERN_INFO "in function %s : cmd = %u\n", __func__, cmd);
	switch (cmd) {
	case GSENSOR_IOCTL_APP_SET_AFLAG:
		...
		if (temp < 0 || temp > 1)
			return -EINVAL;
		printk(KERN_INFO "xiongdajun %s: ioctl set active,value:%d\n",
		       __func__, temp);
		if (temp) {
			lis3dh_acc_enable(lis3dh_acc);
		} else {
			lis3dh_acc_disable(lis3dh_acc);
		}
		break;
	case GSENSOR_IOCTL_APP_SET_DELAY:
		if (copy_from_user(&temp, argp, sizeof(temp)))
			return -EFAULT;
		printk(KERN_INFO "%s: ioctl set delay,value = %d\n", __func__,
		       temp);
		acc_delay_ms = temp;
		break;
		...
		/*case LIS3DH_IOCTL_GET_DELAY:
		   ...
		   return -EFAULT;
		   printk("%s:ioctl get delay,value:%d\n",__func__,temp);
		   break;
		 */
	case GSENSOR_IOCTL_APP_CALIBRATE:
		/* read from flash */
		printk(KERN_INFO "%s: Calibrate start\n", __func__);

		if (!lis3dh_acc->hw_initialized) {
			err = lis3dh_acc_hw_init(lis3dh_acc);
			if (err < 0) {
				dev_err(&lis3dh_acc->client->dev,
				"%s: hw init error!\n", __func__);
				return err;
			}
		}
		ret = lis3dh_acc_Calibration(10);
		printk(KERN_INFO "%s zbtest: ret = %d\n", __func__, ret);
		if (ret < 0)
			dev_err(&lis3dh_acc->client->dev,
				"%s: set default offset!\n", __func__);

		((struct acc_offset *)acc_cal_ptr)->x =
		    lis3dh_acc->accel_cali[acc->pdata->axis_map_x];
		((struct acc_offset *)acc_cal_ptr)->y =
		    lis3dh_acc->accel_cali[acc->pdata->axis_map_y];
		((struct acc_offset *)acc_cal_ptr)->z =
		    lis3dh_acc->accel_cali[acc->pdata->axis_map_z];
		((struct acc_offset *)acc_cal_ptr)->key = ret ? 2 : 1;

		printk(KERN_INFO "%s write: xoffset = %d\n", __func__,
		       ((struct acc_offset *)acc_cal_ptr)->x);
		printk(KERN_INFO "%s write: yoffset = %d\n", __func__,
		       ((struct acc_offset *)acc_cal_ptr)->y);
		printk(KERN_INFO "%s write: zoffset = %d\n", __func__,
		       ((struct acc_offset *)acc_cal_ptr)->z);
		printk(KERN_INFO "%s write: CalStatus = %d\n", __func__,
		       ((struct acc_offset *)acc_cal_ptr)->key);
		sensparams_write_to_flash(SENSPARAMS_TYPE_ACC,
					  (unsigned char *)acc_cal_ptr,
					  sizeof(acc_cal_data));
		if (copy_to_user
		    ((struct acc_offset *)arg, acc_cal_ptr,
		     sizeof(acc_cal_data))) {
			dev_err(&lis3dh_acc->client->dev,
				"%s:  Calibrate copy_to_user failed!\n",
				__func__);
			return -EFAULT;
		}
	...
	case GSENSOR_IOCTL_APP_OFFSET:
		...
		{
			printk(KERN_INFO
			       "%s:  get offset copy_to_user failed!\n",
			       __func__);
			return -EFAULT;
		}
		break;
			    ...
			    const struct i2c_device_id *id)
{
	...
	struct lis3dh_acc_data *acc;
	//xiongdajun add start 20130620
#ifdef CONFIG_OF
	struct lis3dh_acc_platform_data *accel_pdata;
#else
	const struct lis3dh_acc_platform_data *accel_pdata =
...
/*	if (client->dev.platform_data == NULL) {
	...
	if (client->dev.of_node) {
		accel_pdata = devm_kzalloc(&client->dev,
					   sizeof(struct
						  lis3dh_acc_platform_data),
					   GFP_KERNEL);
		if (!accel_pdata) {
			dev_err(&client->dev, "Failed to allocate memory\n");
			return -ENOMEM;
		}
		err = lis3dh_accel_parse_dt(&client->dev, accel_pdata);
		if (err)
			return err;
	} else {
...
/*	acc->pdata = kmalloc(sizeof(*acc->pdata), GFP_KERNEL);
	...
	if (acc->pdata->gpio_int1 >= 0) {
		...
		printk(KERN_INFO "%s: %s has mapped irq1 on gpio: %d\n",
		       LIS3DH_ACC_DEV_NAME, __func__, acc->pdata->gpio_int1);
	}
	...
	if (acc->pdata->gpio_int2 >= 0) {
		...
		printk(KERN_INFO "%s: %s has mapped irq2 on gpio: %d\n",
		       LIS3DH_ACC_DEV_NAME, __func__, acc->pdata->gpio_int2);
	}
...
static struct i2c_driver lis3dh_acc_driver = {
	.driver = {
		   .owner = THIS_MODULE,
		   .name = GSENSOR_DEVICE_NAME,
		   /* added by xiongdajun for device tree, start */
#ifdef CONFIG_OF
		   .of_match_table = bosch_of_match,
#endif
		   /* added by xiongdajun for device tree, end */
