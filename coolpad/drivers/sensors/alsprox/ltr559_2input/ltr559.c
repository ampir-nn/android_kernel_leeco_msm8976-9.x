...
#endif
#include <linux/sensors/ltr559_2input.h>
#include <linux/sensors/sensparams.h>

#include <linux/sensors.h>

#ifdef CONFIG_YL_SENSORS_DEBUG
	#define LTR_DBG(fmt, args...) pr_info(fmt, ##args)
#else
	#define LTR_DBG(fmt, args...)
#endif
...
//Yulong add for debug
static int als_adjust_low_debug = 0;
module_param_named(
        als_low_debug, als_adjust_low_debug, int, S_IRUGO | S_IWUSR | S_IWGRP
);

static int als_adjust_high_debug = 0;
module_param_named(
        als_high_debug, als_adjust_high_debug, int, S_IRUGO | S_IWUSR | S_IWGRP
);
...
static int ltr559_i2c_read(struct ltr559_data *ltr559,u8 regnum,u8 *data)
{
	...
	do {
		...
		if (0 > readdata) {
			LTR_DBG("%s: i2c_read failed err = %d\n", __func__, readdata);
...
static int ltr559_light_enable(struct ltr559_data *ltr559)
{
	...
	ret = ltr559_i2c_write(ltr559, LTR559_ALS_CONTR, LTR559_MODE_ALS_ON_Range8);
	if (ret) {
...
static int ltr559_ps_read(struct ltr559_data *ltr559, int *psdata)
{
	...
	*psdata = ((psval_hi & 7)* 256) + psval_lo;
	...
	/*LTR_DBG("%s: psval_lo:%d psval_hi:%d psdata:%d",
...
static int ltr559_als_read(struct ltr559_data *ltr559, unsigned int *lux_value)
{
	...
	if ((alsval_ch0 + alsval_ch1) == 0) {
		//LTR_DBG("Both CH0 and CH1 are zero\n");
		ratio = 0;
	} else {
	...
	*lux_value *= 2;
	...
	//LTR_DBG("%s: alsval_ch0[%d], alsval_ch1[%d], lux_value:%d\n ", __func__, alsval_ch0, alsval_ch1, *lux_value);
	return err;
}
...
static void ltr559_work_func_light(struct work_struct *work)
{
	...
	if (heart_reg == 0xA0) {
		...
		while (heart_flag == 1) {
			...
			input_event(ltr559->input_dev_light, EV_MSC, MSC_SCAN, heartrate);
			input_sync(ltr559->input_dev_light);
		}
	} else {
		err = ltr559_als_read(ltr559, &lux_val);
...
static int ltr559_parse_dt(struct device *dev, struct ltr559_platform_data *pdata)
{
	...
	// pdata->ltr559_prox_factory_offset    = array[index++];

	LTR_DBG("%s: prox_threshold_hi=%d, prox_threshold_lo=%d\n", __func__, pdata->prox_threshold_hi, pdata->prox_threshold_lo);
	LTR_DBG("%s: prox_factory_threshold_hi=%d, prox_factory_threshold_lo=%d\n", __func__, pdata->prox_factory_threshold_hi, pdata->prox_factory_threshold_lo);
	LTR_DBG("%s: gpio_int=%d, irq_gpio_flags=%d\n", __func__, pdata->gpio_int, pdata->irq_gpio_flags);
	LTR_DBG("%s: ltr559_interrupt_persist=%d\n", __func__, pdata->interrupt_persist);
	LTR_DBG("%s: ltr559_ps_led=%d\n", __func__, pdata->ps_led);
	LTR_DBG("%s: ltr559_ps_n_pulses=%d\n", __func__, pdata->ps_n_pulses);
	LTR_DBG("%s: ltr559_ps_meas_rate=%d\n", __func__, pdata->ps_meas_rate);
	LTR_DBG("%s: ltr559_als_meas_rate=%d\n", __func__, pdata->als_meas_rate);
	// LTR_DBG("%s: ltr559_prox_factory_offset=%d\n", __func__, pdata->ltr559_prox_factory_offset);
...
static int ltr559_get_data(void)
{
	...
	newdata = status & 0x05;
	LTR_DBG("%s: interrupt_stat = %d, newdata = %d\n", __func__, interrupt_stat, newdata);
...
{
	struct ltr559_data *ltr559 = ltr559_dev;
#ifdef CONFIG_BOARD_CP5961
        if (200 > struct_noise) {
                ltr559->pdata->prox_threshold_lo = struct_noise+45;
                ltr559->pdata->prox_threshold_hi = struct_noise+65;
        } else if (400 > struct_noise) {
                ltr559->pdata->prox_threshold_lo = struct_noise+55;
                ltr559->pdata->prox_threshold_hi = struct_noise+85;
        } else if (600 > struct_noise) {
                ltr559->pdata->prox_threshold_lo = struct_noise+70;
                ltr559->pdata->prox_threshold_hi = struct_noise+95;
        } else if (800 > struct_noise) {
                ltr559->pdata->prox_threshold_lo = struct_noise+85;
                ltr559->pdata->prox_threshold_hi = struct_noise+115;
        } else if (ltr559->pdata->ltr559_struct_noise_threshold > struct_noise) {
                ltr559->pdata->prox_threshold_lo = struct_noise+120;
                ltr559->pdata->prox_threshold_hi = struct_noise+150;
	}else{
                ltr559->pdata->prox_threshold_lo = struct_noise+140;
                ltr559->pdata->prox_threshold_hi = struct_noise+180;
                printk(KERN_ERR "%s: Calibration fail, prox_avg = [%d]\n", __func__, struct_noise);
                return -1;
        }
#else
	if (90 > struct_noise) {
		ltr559->pdata->prox_threshold_lo = struct_noise+ltr559->pdata->ltr559_proximity_low_offset0 + als_adjust_low_debug;
		ltr559->pdata->prox_threshold_hi = struct_noise+ ltr559->pdata->ltr559_proximity_high_offset0 + als_adjust_high_debug;
	} else if (255 > struct_noise) {
		ltr559->pdata->prox_threshold_lo = struct_noise+ ltr559->pdata->ltr559_proximity_low_offset1 + als_adjust_low_debug;
		ltr559->pdata->prox_threshold_hi = struct_noise+ltr559->pdata->ltr559_proximity_high_offset1 + als_adjust_high_debug;
	} else if (ltr559->pdata->ltr559_struct_noise_threshold > struct_noise) {
...
static long ltr559_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case LTR559_IOCTL_ALS_ON:
		...
		LTR_DBG("%s: ALS_ON exit\n", __func__);
		break;

	case LTR559_IOCTL_ALS_OFF:
		LTR_DBG("%s: ALS_OFF Entry\n", __func__);
		ltr559_dev->als_on = 0;
		...
		LTR_DBG("%s: ALS_OFF exit\n", __func__);
		break;

	case LTR559_IOCTL_PROX_ON:
		LTR_DBG("%s: PROX_ON Entry, hi=%d, lo=%d, factory_hi=%d, factory_lo=%d\n", __func__, ltr559->pdata->prox_threshold_hi,
		...
		LTR_DBG("%s: PROX_ON exit\n", __func__);
	  	break;

	  case LTR559_IOCTL_PROX_OFF:
		LTR_DBG("%s: PROX_OFF Entry\n", __func__);
		...
		LTR_DBG("%s: PROX_OFF exit\n", __func__);
	  	break;

	case LTR559_IOCTL_PROX_CALIBRATE:
		LTR_DBG("%s: PROX_CALIBRATE Entry\n", __func__);
		...
		LTR_DBG("%s: PROX_CALIBRATE exit\n", __func__);
	  	break;

	  case LTR559_IOCTL_PROX_OFFSET:
		LTR_DBG("%s: PROX_OFFSET Entry\n", __func__);
		...
		LTR_DBG("%s: PROX_OFFSET prox_version = %d\n", __func__, ltr559->pdata->prox_version);
		LTR_DBG("%s: PROX_OFFSET prox_high = %d\n", __func__, ltr559->pdata->prox_threshold_hi);
		LTR_DBG("%s: PROX_OFFSET prox_low = %d\n", __func__, ltr559->pdata->prox_threshold_lo);
		LTR_DBG("%s: PROX_OFFSET prox_factory_high = %d\n", __func__, ltr559->pdata->prox_factory_threshold_hi);
		LTR_DBG("%s: PROX_OFFSET prox_factory_low = %d\n", __func__, ltr559->pdata->prox_factory_threshold_lo);
			  ...
			  const struct i2c_device_id *id)
{
	...
	LTR_DBG("%s: registering input device\n", __func__);
