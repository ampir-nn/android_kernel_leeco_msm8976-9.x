...
#include <linux/sensors/pa12200001_hq.h>
...
#define PA12_IOCTL_ALS_ON32            ALSPROX_IOCTL_ALS_ON32
#define PA12_IOCTL_ALS_OFF32           ALSPROX_IOCTL_ALS_OFF32
#define PA12_IOCTL_PROX_ON32           ALSPROX_IOCTL_PROX_ON32
#define PA12_IOCTL_PROX_OFF32          ALSPROX_IOCTL_PROX_OFF32
#define PA12_IOCTL_PROX_CALIBRATE32    ALSPROX_IOCTL_PROX_CALIBRATE32
#define PA12_IOCTL_PROX_OFFSET32       ALSPROX_IOCTL_PROX_OFFSET32
#define PA12_IOCTL_PHONE_STATE32       ALSPROX_IOCTL_PHONE_STATE32
...
static long pa12200001_ioctl_compat(struct file *file, unsigned int cmd, unsigned long arg)
{
...
//  int init_ps_gain = 0;
	...
	struct prox_offset pa12200001_cal_data;
	struct prox_offset *pa12200001_cal_ptr = &pa12200001_cal_data;
	struct prox_status mStatus = {0, 0};
    u8 first_open =1 ;

     switch (cmd) {
        case PA12_IOCTL_ALS_ON32:
            pa12200001_als_enable(1);
            light_active = 1;
            APS_LOG("[pa12]:>>>>%s: enable pa12_als light \n", __func__);
       break;
        case PA12_IOCTL_ALS_OFF32:
            pa12200001_als_enable(0);
            light_active = 0;
  /*
    ...
    */
        ...
        case PA12_IOCTL_PROX_ON32:
...

            {
              ...
              printk("-!!!!-chenyz!!!!:val=%d,first_open=%d,temp_flag=%d\n",val,first_open,temp_flag);
            

              if(((temp_flag==1)&&(first_open==1))||((temp_flag != 1)&&(temp_flag == val)))//temp is not exit the first time
              {

                if(temp_flag!=3){//33 33 33 
        ...
        case PA12_IOCTL_PROX_OFF32:
            ...
            break;
        case PA12_IOCTL_PROX_CALIBRATE32:
			...
			((struct prox_offset *)pa12200001_cal_ptr)->x = (unsigned short)(this_data->pdata->pa12_ps_th_high);
			((struct prox_offset *)pa12200001_cal_ptr)->y = (unsigned short)(this_data->pdata->pa12_ps_th_low);
			((struct prox_offset *)pa12200001_cal_ptr)->z = (unsigned short)(this_data->crosstalk);
			((struct prox_offset *)pa12200001_cal_ptr)->offset = 0;
			((struct prox_offset *)pa12200001_cal_ptr)->key = xtalk;

			if(copy_to_user((struct prox_offset *)arg, pa12200001_cal_ptr, sizeof(pa12200001_cal_data))) {
...
#ifdef CONFIG_COMPAT
	.compat_ioctl   = pa12200001_ioctl_compat,
#endif
