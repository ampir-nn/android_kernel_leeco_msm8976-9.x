...

{
	...
	if (flag) {
		...
		i2c_write_reg(this_data->client, REG_CFG0, regdata | 0x01);
		mutex_unlock(&this_data->lock);
		if (als_polling)
			hrtimer_start(&this_data->light_timer,
				this_data->light_poll_delay, HRTIMER_MODE_REL);
	}else{
		mutex_lock(&this_data->lock);
		i2c_read_reg(this_data->client, REG_CFG0, &regdata);
...
static int pa12200001_run_calibration(struct i2c_client *client)
{
 ...
 RECALIBRATION:
    ...
    for (i = 5; i < 15; i++)
   {
	...
	mutex_lock(&data->lock);//csl modify
	/*Write offset value to 0x10*/
	ret = i2c_write_reg(client, REG_PS_OFFSET, data->crosstalk);
	/*Restore Original Status*/

	ret = i2c_write_reg(client,REG_CFG0,cfg0data); //restore CFG0 Initail status
	ret = i2c_write_reg(client,REG_CFG2,cfg2data | 0xC0); //restore CFG2 Initail status
...
static int pa12200001_fast_run_calibration(struct i2c_client *client)
{
	...
	if((data->crosstalk < xtalk_data) && (xtalk_data < 100)){    //Max Value is 150,and must be grater than x-talk cal value
		APS_LOG("%s: sum_of_pdata = %d   cross_talk = %d\n",
	...
	ret = i2c_write_reg(client,REG_CFG0,cfg0data); //restore CFG0 Initail status
	ret = i2c_write_reg(client,REG_CFG2,cfg2data | 0xC0); //restore CFG2 Initail status
	ret = i2c_write_reg(client,REG_CFG3,cfg3data); //restore CFG3 Initail status
	i2c_read_reg(client,REG_CFG0,&cfg0data);
	i2c_read_reg(client,REG_CFG2,&cfg2data);
	i2c_read_reg(client,REG_CFG3,&cfg3data);
	APS_LOG("cfg0 = %x\n cfg2 = %x\n cfg3 = %x\n",cfg0data, cfg2data, cfg3data);
...
static int pa12200001_parse_configs(struct device *dev, char *name, u32 *array)
{
	...
	if(rc&&(rc!=-EINVAL)){
	dev_err(dev,"%s:Unable to read device tree\n",__func__);
	return rc;
	}
	dev_info(dev,"size is %d\n", prop->length/sizeof(u32));
	for(i = 0; i < prop->length/sizeof(u32); i++){
		dev_info(dev,"array[%d] = %d",i ,array[i]);
	}
	dev_info(dev, "\n");
	return rc;
}
...
static void pa12200001_work_func_proximity(struct work_struct *work)
{
	...
	input_event(data->input_dev, EV_MSC, MSC_SCAN, Pval);
...
static void pa12200001_work_func_light(struct work_struct *work)
{
	...
	pr_debug("ALS lux value: %d\n", Aval);
	input_report_abs(data->input_dev, ABS_MISC, Aval);
	/*input_event(data->input_dev, EV_MSC, MSC_SCAN, Aval);wujinrong del*/
	input_sync(data->input_dev);
}
...
static int pa12200001_init_client(struct i2c_client *client)
{
	...
	int ret;
  	...
  	ret=i2c_write_reg(client,REG_CFG1,
		((data->pdata->pa12_led_curr << 4)|
			 (data->pdata->pa12_ps_prst << 2)| (data->pdata->pa12_als_prst)));// LED Current,PS PRST,ALS PRST
  	...
  	ret=i2c_write_reg(client,REG_CFG3,
		((data->pdata->pa12_int_type << 6)|
			 (data->pdata->pa12_ps_period << 3)| (data->pdata->pa12_als_period)));//Interrupt Type
  ...
  /*
	...
	*/
		...
		case PA12_IOCTL_PROX_ON:
			...
			queue_work(this_data->wq, &this_data->work_proximity);//force PS report system  status
			*/
		...
		case PA12_IOCTL_PROX_OFFSET:
			...
			sensparams_read_from_flash(SENSPARAMS_TYPE_PROX,
					prox_param, 4);
			mutex_lock(&this_data->lock);
			i2c_write_reg(this_data->client,
				REG_PS_OFFSET, prox_param[3]);
			mutex_unlock(&this_data->lock);
			break;
...
static int pa12200001_open(struct inode *inode, struct file *file)
{
	...
	mutex_lock(&this_data->lock);
	i2c_write_reg(this_data->client, REG_PS_OFFSET,
			data->pdata->pa12_ps_offset_default);
	mutex_unlock(&this_data->lock);
	data->crosstalk = data->pdata->pa12_ps_offset_default;
    }
		...
		const struct i2c_device_id *id)
{
	...
	if (!data->input_dev) {
		pr_err("%s: allocate input device fail !\n", __func__);
		ret = -1;
		goto exit_kfree;
	}
	data->input_dev->name = PA12200001_INPUT_NAME;
	...
	/*light lux data*/
	input_set_abs_params(data->input_dev, ABS_MISC, 0, 100000, 0, 0);
    /* light Lux and proximity data */
	...
	if (!als_polling || !ps_polling)//if lianzhezhongyouyigeweiyi
	{
		...
		err = request_irq(data->als_ps_int,pa12200001_irq,
