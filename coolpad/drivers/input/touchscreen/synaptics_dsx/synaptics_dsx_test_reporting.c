...
#define CALIBRATION_TIMEOUT_S 15
...
static int test_do_afe_calibration(enum f54_afe_cal mode)
{
	...
	dev_info(rmi4_data->pdev->dev.parent,
			"%s:calibration crc = %d, is_crc = %d.\n",
			__func__, f54->data_31.calibration_crc,
			f54->data_31.is_calibration_crc);
...
/* add for tp fastmmi test--begin */
int test_synaptics_need_do_calibration(void)
{
	int retval;
	struct synaptics_rmi4_data *rmi4_data = f54->rmi4_data;
...
int test_synaptics_do_calibration(void)
{
	int retval;
	/*just do normal calibration.*/
	retval = test_do_afe_calibration(F54_AFE_CAL);

	return retval;
}

/* add for tp fastmmi test--end */
