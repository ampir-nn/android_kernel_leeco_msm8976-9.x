...
#include <linux/input/synaptics_dsx_vendor.h>
...
/* add for fastmmi test: begin*/
unsigned char sensor_id = 0; /* we can't read sensor id now */
unsigned char ic_name[10];
...
/* return 0: no need calibration
** return 1: need do calibration
*/
int syna_tp_need_calibrate(void)
{
	pr_info("%s...\n", __func__);
	return test_synaptics_need_do_calibration();
}
...
{
	pr_info("%s...\n", __func__);
	return test_synaptics_do_calibration();
}
...
static int syna_read_ic_name(void)
{
	int retval;
	struct synaptics_rmi4_data *rmi4_data = fwu->rmi4_data;
	...
	if (retval < 0) {
		dev_dbg(rmi4_data->pdev->dev.parent,
			"%s: Failed to read IC name from reg.\n",
			__func__);
	}

	dev_dbg(rmi4_data->pdev->dev.parent,
			"%s: IC name:%d, %d, %d, %d, %d\n",
			__func__, ic_name[0], ic_name[1], ic_name[2],
			ic_name[3], ic_name[4]);
...
int syna_tp_get_version(char *version)
{
	...
	if (!strcmp(ic_name, "S332U")) {
		return snprintf(version, PAGE_SIZE, "%s:%s:%d:%d",
			"Truly", "S332U", fwu->rmi4_data->firmware_id,
			fwu->config_id[3]);
	} else
		return snprintf(version, PAGE_SIZE, "%s:%s:%d:%d",
			"NA", "NA", 0, 0);
...
{
	return snprintf(vendor, PAGE_SIZE, "%s%x", "synaptics_", sensor_id);
}

int syna_ftsscaptest(char *result)
{
	pr_info("%s...\n", __func__);
	return 1;
}
...
struct touchscreen_funcs syna_tp_ops = {
	...
	.ftsscaptest		= syna_ftsscaptest,
};
/* add for fastmmi test: end */
