...
static char g_pStoreAllData[1024*32] = {0};
...
static bool  AllocateMemory(void)
{
	...
	if (NULL == g_pStoreMsgArea) {
		g_pStoreMsgArea = kzalloc(1024 * 8, GFP_KERNEL);
		if (NULL == g_pStoreMsgArea) {
			printk(KERN_ERR "[focal] 1:alloc memory fail\n");
	...
	if (NULL == g_pMsgAreaLine2) {
		g_pMsgAreaLine2 = kzalloc(1024 * 8, GFP_KERNEL);
		if (NULL == g_pMsgAreaLine2) {
			printk(KERN_ERR "[focal] 2:alloc memory fail\n");
	...
	if (NULL == g_pStoreDataArea) {
		g_pStoreDataArea = kzalloc(1024 * 8, GFP_KERNEL);
		if (NULL == g_pStoreDataArea) {
			printk(KERN_ERR "[focal] 3:alloc memory fail\n");
	...
	g_pTmpBuff = NULL;
	if (NULL == g_pTmpBuff) {
		g_pTmpBuff = kmalloc(1024 * 4, GFP_KERNEL);
		if (NULL == g_pTmpBuff) {
			printk(KERN_ERR "[focal] 4:alloc memory fail\n");
			goto err_pTmpBuff;
		}
	}
	return true;
...
unsigned char GetRawData(void)
{
	...
	//--------------------------------------------Start Scanning
	printk("[focal] Start Scan ...\n");
	ReCode = StartScan();
	if(ERROR_CODE_OK != ReCode)
	{
		printk("[focal] Failed to Scan ...\n");
		return ReCode;
	}
	...
	if(ReCode != ERROR_CODE_OK)
	{
		printk("[focal] Failed to Read RawData...\n");
		return ReCode;
	}
...
unsigned char FT6X36_TestItem_RawDataTest(bool * bTestResult)
{
	...
	{
		printk("[focal] RawData Test is Error. Failed to get Raw Data!!\n");
	...
	printk("// Max Raw Value: %d, Min Raw Value: %d, Deviation Value: %d, Average Value: %d\n", iMax, iMin, iMax - iMin, iAvg);
...
TEST_END:
	...
	{
		* bTestResult = true;
		printk("[focal] RawData Test is OK!\n");
	}
	else
	{
		* bTestResult = false;
		printk("[focal] RawData Test is NG!\n");
	}
	return ReCode;
}
...
unsigned char FT6X36_TestItem_CbTest(bool * bTestResult)
{
	...
	{
		btmpresult = false;
		printk("//=========  CB test Failed!\n");
	}
	else
	{
		printk("[focal] Get Proof_NoWaterProof CB Data...\n");
	...
	////////////////////////////单端防水
	printk("[focal] Proof_Level0 CB Test...\n");
	for(i = 0; i < 3; i++)
	{
	...
	{
		printk("[focal] Get Proof_Level0 CB Data...\n");
		...
		iAvg /= g_ScreenSetParam.iChannelsNum + g_ScreenSetParam.iKeyNum;
		printk("// Max CB Value: %d, Min CB Value: %d, Deviation Value: %d, Average Value: %d\n", iMax, iMin, iMax - iMin, iAvg);

		if(btmpresult)
		{
			printk("[focal] CB Test is OK!\n");
			* bTestResult = 1;
		}
		else
		{
			printk("[focal] CB Test is NG!\n");
			* bTestResult = 0;
		}
	}
	////>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>收集测试数据，存入CSV文件
...
unsigned char FT6X36_TestItem_DeltaCbTest(unsigned char * bTestResult)
{
	...
	printk("==========================Test Item: -------- Delta CB Test\n");
	...
	for(i=0; i < readlen; i++)
	{
		...
		if(i== 0)//Half
		{
			printk("//======= Differ Data of Delta CB:\n");
			printk("Left Channel:\n");
		}
		else if( i * 2 == g_ScreenSetParam.iChannelsNum)
		{
	...
	if(bUseSort1)
	{
		if(Delta_Ci_Differ <= Sort1Max - Sort1Min)
		{
			if(bUseCriticalValue)
			{
				...
				printk("// Condition1: Max of Delta CB_Differ: %d, Min of Delta CB_Differ: %d, Get Deviation: %d, Set Critical Deviation of Sort1: %d\n",
					Sort1Max, Sort1Min, Sort1Max - Sort1Min, Critical_Delta_S1);
			}
			else
			{
				btmpresult = false;
				printk("// Condition1: Max of Delta CB_Differ: %d, Min of Delta CB_Differ: %d, Get Deviation: %d, Set Deviation of Sort1: %d\n",
	...
	if(bUseSort2)
	{
		...
		if(Delta_Ci_Differ_S2 <= Sort2Max - Sort2Min)
		{
			if(bUseCriticalValue)
			{
				...
				printk("// Condition1: Max of Delta CB_Differ: %d, Min of Delta CB_Differ: %d, Get Deviation: %d, Set Critical Deviation of Sort2: %d\n",
					Sort2Max, Sort2Min, Sort2Max - Sort2Min, Critical_Delta_S2);
			}
			else
			{
				btmpresult = false;
				printk("// Condition1: Max of Delta CB_Differ: %d, Min of Delta CB_Differ: %d, Get Deviation: %d, Set Deviation of Sort2: %d\n",
	...
	if(bUseSort3)
	{
		...
		if(Delta_Ci_Differ_S3 <= Sort3Max - Sort3Min)
		{
			if(bUseCriticalValue)
			{
				...
				printk("// Condition1: Max of Delta CB_Differ: %d, Min of Delta CB_Differ: %d, Get Deviation: %d, Set Critical Deviation of Sort3: %d\n",
					Sort3Max, Sort3Min, Sort3Max - Sort3Min, Critical_Delta_S3);
			}
			else
			{
				btmpresult = false;
				printk("// Condition1: Max of Delta CB_Differ: %d, Min of Delta CB_Differ: %d, Get Deviation: %d, Set Deviation of Sort3: %d\n",
	...
	if(bUseSort4)
	{
		if(Delta_Ci_Differ_S4 <= Sort4Max - Sort4Min)
		{
			if(bUseCriticalValue)
			{
				...
				printk("// Condition1: Max of Delta CB_Differ: %d, Min of Delta CB_Differ: %d, Get Deviation: %d, Set Critical Deviation of Sort4: %d\n",
			...
			{
				btmpresult = false;
				printk("// Condition1: Max of Delta CB_Differ: %d, Min of Delta CB_Differ: %d, Get Deviation: %d, Set Deviation of Sort4: %d\n",
	...
	if(bUseSort5)
	{
		if(Delta_Ci_Differ_S5 <= Sort5Max - Sort5Min)
		{
			if(bUseCriticalValue)
			{
				...
				printk("// Condition1: Max of Delta CB_Differ: %d, Min of Delta CB_Differ: %d, Get Deviation: %d, Set Critical Deviation of Sort5: %d\n",
			...
			{
				btmpresult = false;
				printk("// Condition1: Max of Delta CB_Differ: %d, Min of Delta CB_Differ: %d, Get Deviation: %d, Set Deviation of Sort5: %d\n",
	...
	if(bUseSort6)
	{
		if(Delta_Ci_Differ_S6 <= Sort6Max - Sort6Min)
		{
			if(bUseCriticalValue)
			{
				...
				printk("// Condition1: Max of Delta CB_Differ: %d, Min of Delta CB_Differ: %d, Get Deviation: %d, Set Critical Deviation of Sort6: %d\n",
					Sort6Max, Sort6Min, Sort6Max - Sort6Min, Critical_Delta_S6);
			}
			else
			{
				btmpresult = false;
				printk("// Condition1: Max of Delta CB_Differ: %d, Min of Delta CB_Differ: %d, Get Deviation: %d, Set Deviation of Sort6: %d\n",
	...
	printk("Condition2:Get Max Differ Data of Delta_CB(abs): %d, Set Max Differ Data of Delta_CB(abs): %d\n", Delta_Max, set_Delta_Cb_Max);
	...
	if(g_stCfg_FT6X36_BasicThreshold.DeltaCbTest_Include_Key_Test)
	{
		...
		printk("// Condition3: Include Key Test, Get Max Key Data: %d, Set Max Key Data: %d\n", Key_Delta_Max, SetKeyMax);
	}

	printk("Max Differ Data of Delta_CB(abs): %d\n", Delta_Max);

	if(bCriticalResult && btmpresult)
	{
		printk("Delta CB Test has Critical Result(TBD)!\n");
	}
	///////////////////////////////////////////////////////Delta Ci End
	if(btmpresult)
	{
		printk("//Delta CB Test is OK!\n");
...
unsigned char FT6X36_TestItem_ChannelsDeviationTest(unsigned char * bTestResult)
{
	...
	printk("==============================Test Item: -------- Channels Deviation Test\n");
	...
	for(i=0; i < g_ScreenSetParam.iChannelsNum/*readlen*/; i++)
	{
					...
					else
					{
						...
						if(GetDeviation >= DeviationMax_S1)
						{
							if(bUseCriticalValue)
							{
								...
								{
									btmpresult = false;
									printk("Ch_%2d, Value: %2d,	Ch_%2d, Value: %2d,	Deviation: %d,	Set Max Critical Deviation 0f Sort1: %d\n",
							...
							{
								btmpresult = false;
								printk("Ch_%2d, Value: %2d,	Ch_%2d, Value: %2d,	Deviation: %d,	Set Max Deviation 0f Sort1: %d\n",
				...
				if(Sort2LastNum + 1 == i)//相邻通道
				{
					...
					if(GetDeviation >= DeviationMax_S2)
					{
						if(bUseCriticalValue)
						{
							...
							{
								btmpresult = false;
								printk("Ch_%2d, Value: %2d,	Ch_%2d, Value: %2d,	Deviation: %d,	Set Max Critical Deviation 0f Sort2: %d\n",
						...
						{
							btmpresult = false;
							printk("Ch_%2d, Value: %2d,	Ch_%2d, Value: %2d,	Deviation: %d,	Set Max Deviation of Sort2: %d\n",
				...
				if(Sort3LastNum + 1 == i)//相邻通道
				{
					...
					if(GetDeviation >= DeviationMax_S3)
					{
						if(bUseCriticalValue)
						{
							...
							{
								btmpresult = false;
								printk("Ch_%2d, Value: %2d,	Ch_%2d, Value: %2d,	Deviation: %d,	Set Max Critical Deviation 0f Sort3: %d\n",
						...
						{
							btmpresult = false;
							printk("Ch_%2d, Value: %2d,	Ch_%2d, Value: %2d,	Deviation: %d,	Set Max Deviation of Sort3: %d\n",
				...
				if(Sort4LastNum + 1 == i)//相邻通道
				{
					...
					if(GetDeviation >= DeviationMax_S4)
					{
						if(bUseCriticalValue)
						{
							...
							{
								btmpresult = false;
								printk("Ch_%2d, Value: %2d,	Ch_%2d, Value: %2d,	Deviation: %d,	Set Max Critical Deviation 0f Sort4: %d\n",
						...
						{
							btmpresult = false;
							printk("Ch_%2d, Value: %2d,	Ch_%2d, Value: %2d,	Deviation: %d,	Set Max Deviation of Sort4: %d\n",
				...
				if(Sort5LastNum + 1 == i)//相邻通道
				{
					...
					if(GetDeviation >= DeviationMax_S5)
					{
						if(bUseCriticalValue)
						{
							...
							{
								btmpresult = false;
								printk("Ch_%2d, Value: %2d,	Ch_%2d, Value: %2d,	Deviation: %d,	Set Max Critical Deviation 0f Sort5: %d\n",
						...
						{
							btmpresult = false;
							printk("Ch_%2d, Value: %2d,	Ch_%2d, Value: %2d,	Deviation: %d,	Set Max Deviation of Sort5: %d\n",
				...
				if(Sort6LastNum + 1 == i)//相邻通道
				{
					...
					if(GetDeviation >= DeviationMax_S6)
					{
						if(bUseCriticalValue)
						{
							...
							{
								btmpresult = false;
								printk("Ch_%2d, Value: %2d,	Ch_%2d, Value: %2d,	Deviation: %d,	Set Max Critical Deviation 0f Sort6: %d\n",
						...
						{
							btmpresult = false;
							printk("Ch_%2d, Value: %2d,	Ch_%2d, Value: %2d,	Deviation: %d,	Set Max Deviation of Sort6: %d\n",
		...
		if(!bFirstUseSort1)
		{
			printk("Get max deviation of Sort1: %d,  Set max deviation of Sort1: %d\n", MaxDev_AllS1, DeviationMax_S1);

		}
		if(!bFirstUseSort2)
		{
			printk("Get max deviation of Sort2: %d,  Set max deviation of Sort2: %d\n", MaxDev_AllS2, DeviationMax_S2);

		}
		if(!bFirstUseSort3)
		{
			printk("Get max deviation of Sort3: %d,  Set max deviation of Sort3: %d\n", MaxDev_AllS3, DeviationMax_S3);

		}
		if(!bFirstUseSort4)
		{
			printk("Get max deviation of Sort4: %d,  Set max deviation of Sort4: %d\n", MaxDev_AllS4, DeviationMax_S4);

		}
		if(!bFirstUseSort5)
		{
			printk("Get max deviation of Sort5: %d,  Set max deviation of Sort5: %d\n", MaxDev_AllS5, DeviationMax_S5);

		}
		if(!bFirstUseSort6)
		{
			printk("Get max deviation of Sort6: %d,  Set max deviation of Sort6: %d\n", MaxDev_AllS6, DeviationMax_S6);

		}

		printk("Max Deviation,\n");
		if(!bFirstUseSort1)
		{
		...
		if(bCriticalResult && btmpresult)
		{
			printk("Channels Deviation Test has Critical Result(TBD)!\n");
		}
	///////////////////////////////////////////////////////Delta Ci End
	if(btmpresult)
	{
		printk("//Channels Deviation Test is OK!\n");
		* bTestResult = true;
			if(bCriticalResult)
				* bTestResult = 2;
			else
	...
	else
	{
		* bTestResult = false;//NG
		printk("//Channels Deviation Test is NG!\n");
	}
...
unsigned char FT6X36_TestItem_TwoSidesDeviationTest(unsigned char * bTestResult)
{
	...
	printk("==============================Test Item: -------- Two Sides Deviation Test\n");
		...
		if(g_stCfg_SCap_DetailThreshold.DeltaCxTest_Sort[i] == 1)
		{
			...
			if(GetDeviation >= DeviationMax)
			{
				if(bUseCriticalValue)
				{
					...
					{
						btmpresult = false;
						printk("Ch_%2d, Value: %2d,	Ch_%2d, Value: %2d,	Deviation: %d,	Set Max Critical Deviation of Sort1: %d\n",
				...
				{
					btmpresult = false;
					printk("Ch_%2d, Value: %2d,	Ch_%2d, Value: %2d,	Deviation: %d,	Set Max Deviation of Sort1: %d\n",
		...
		if(g_stCfg_SCap_DetailThreshold.DeltaCxTest_Sort[i] == 2)
		{
			...
			if(GetDeviation >= DeviationMax_S2)
			{
				if(bUseCriticalValue)
				{
					...
					{
						btmpresult = false;
						printk("Ch_%2d, Value: %2d,	Ch_%2d, Value: %2d,	Deviation: %d,	Set Max Critical Deviation of Sort2: %d\n",
		...
		if(g_stCfg_SCap_DetailThreshold.DeltaCxTest_Sort[i] == 3)
		{
			...
			if(GetDeviation >= DeviationMax_S3)
			{
				if(bUseCriticalValue)
				{
					...
					{
						btmpresult = false;
						printk("Ch_%2d, Value: %2d,	Ch_%2d, Value: %2d,	Deviation: %d,	Set Max Critical Deviation of Sort3: %d\n",
		...
		if(g_stCfg_SCap_DetailThreshold.DeltaCxTest_Sort[i] == 4)
		{
			...
			if(GetDeviation >= DeviationMax_S4)
			{
				if(bUseCriticalValue)
				{
					...
					{
						btmpresult = false;
						printk("Ch_%2d, Value: %2d,	Ch_%2d, Value: %2d,	Deviation: %d,	Set Max Critical Deviation of Sort4: %d\n",
		...
		if(g_stCfg_SCap_DetailThreshold.DeltaCxTest_Sort[i] == 5)
		{
			...
			if(GetDeviation >= DeviationMax_S5)
			{
				if(bUseCriticalValue)
				{
					...
					{
						btmpresult = false;
						printk("Ch_%2d, Value: %2d,	Ch_%2d, Value: %2d,	Deviation: %d,	Set Max Critical Deviation of Sort5: %d\n",
		...
		if(g_stCfg_SCap_DetailThreshold.DeltaCxTest_Sort[i] == 6)
		{
			...
			if(GetDeviation >= DeviationMax_S6)
			{
				if(bUseCriticalValue)
				{
					...
					{
						btmpresult = false;
						printk("Ch_%2d, Value: %2d,	Ch_%2d, Value: %2d,	Deviation: %d,	Set Max Critical Deviation of Sort6: %d\n",
	...
	if(bUseSort1)
	{
		printk("Get Max Deviation of Sort1: %d,  Set Max Deviation of Sort1: %d\n", DevMax_AllS1, DeviationMax);

	}
	if(bUseSort2)
	{
		printk("Get Max Deviation of Sort2: %d,  Set Max Deviation of Sort2: %d\n", DevMax_AllS2, DeviationMax_S2);

	}
	if(bUseSort3)
	{
		printk("Get Max Deviation of Sort3: %d,  Set Max Deviation of Sort3: %d\n", DevMax_AllS3, DeviationMax_S3);

	}
	if(bUseSort4)
	{
		printk("Get Max Deviation of Sort4: %d,  Set Max Deviation of Sort4: %d\n", DevMax_AllS4, DeviationMax_S4);

	}
	if(bUseSort5)
	{
		printk("Get Max Deviation of Sort5: %d,  Set Max Deviation of Sort5: %d\n", DevMax_AllS5, DeviationMax_S5);

	}
	if(bUseSort6)
	{
		printk("Get Max Deviation of Sort6: %d,  Set Max Deviation of Sort6: %d\n", DevMax_AllS6, DeviationMax_S6);
	...
	if(bCriticalResult && btmpresult)
	{
		printk("Two Sides Deviation Test has Critical Result(TBD)!\n");
	}
	///////////////////////////////////////////////////////Delta Ci End
	if(btmpresult)
	{
		printk("//Two Sides Deviation Test is OK!\n");
		if(bCriticalResult)
			* bTestResult = 2;
		else
			* bTestResult = true;
	...
	{
		* bTestResult = false;//NG
		printk("//Two Sides Deviation Test is NG!\n");
	}
