...
#define GUP_SEARCH_FILE_TIMES       3
...
u8 searching_file;
u8 file_ready = 0;
extern void gtp_reset_guitar(struct goodix_ts_data *ts, s32 ms);
...
#endif
static u8 gup_burn_fw_app_section(struct i2c_client *client,
...
static s32 gup_init_panel(struct goodix_ts_data *ts)
{
	...
	u8 opr_buf[16];
	u8 sensor_id = 0;
	u8 *config = ts->config_data;	/*add by yulong*/
	...
	if (ret == SUCCESS) {
		GTP_DEBUG("CFG_GRP%d Cfg Version: %d, IC Version:%d",
...
static u8 gup_enter_update_judge(st_fw_head *fw_head)
{
	...
	if (!memcmp(fw_head->hw_info, update_msg.ic_fw_msg.hw_info,
		...
		fw_len = 42 * 1024;	/*TODO flashless equals another length*/
	} else {
	...
	if (update_msg.fw_total_len != fw_len) {
		GTP_ERROR("Inconsistent firmware size, Update aborted!\
				Firmware header len: %d, actual size: %d",
			fw_len, update_msg.fw_total_len);
		return FAIL;
	}
...
u8 gup_check_update_file(struct i2c_client *client, st_fw_head *fw_head,
                         u8 *path)
{
	s32 ret = 0;
	s32 i = 0;
	...
	if (path) {
		...
		for (i = 0; i < GUP_SEARCH_FILE_TIMES; i++) {
			...
			if (!(got_file_flag&0x0F)) {
				update_msg.file = filp_open(search_update_path,
				                            O_RDONLY, 0);
				if (!IS_ERR(update_msg.file)) {
...
static u8 gup_load_section_file(u8 *buf, u16 offset, u16 length, u8 set_or_end)
{
	s32 ret = 0;
...
static u8 gup_burn_fw_boot_isp(struct i2c_client *client)
{
	...
	/*ret = gup_load_section_file(fw_boot_isp, (4*FW_SECTION_LENGTH
			+FW_DSP_LENGTH + FW_BOOT_LENGTH + FW_DSP_ISP_LENGTH),
			FW_BOOT_ISP_LENGTH, SEEK_SET);*/
	ret = gup_load_section_file(fw_boot_isp, (update_msg.fw_burned_len
	                            - FW_DSP_ISP_LENGTH), FW_BOOT_ISP_LENGTH, SEEK_SET);
	if(FAIL == ret) {
		GTP_ERROR("[burn_fw_boot_isp]load firmware boot_isp fail.");
		goto exit_burn_fw_boot_isp;
	}

	/*gup_show_elements((4*FW_SECTION_LENGTH + FW_DSP_LENGTH + FW_BOOT_LENGTH
		+ FW_DSP_ISP_LENGTH), fw_boot_isp, 10);*/		/*for test*/
	/*step3:hold ss51 & dsp*/
