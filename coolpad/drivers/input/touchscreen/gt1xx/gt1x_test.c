...
int read_sensorid(void)
{
	...
	return gt1x_fw_version.sensor_id;
}
...
int check_version(unsigned char *ini_version)
{
	...
	ret = strncmp((const char *)gt1x_fw_version.product_id, (const char *)ini_version, 4);
	...
	sprintf((char *)tmp, "%d.%d.%d_%d.%d", (gt1x_fw_version.mask_id >> 16) & 0x0FF, (gt1x_fw_version.mask_id >> 8) & 0x0FF, (gt1x_fw_version.mask_id & 0xFF),
		(gt1x_fw_version.patch_id >> 16) & 0x0FF, (gt1x_fw_version.patch_id & 0xFF));
...
int auto_find_ini(char *findDir, char *format, char *inipath)
{
	...
	sprintf((char *)tmp, "/system/etc/test_sensor_%d.ini", gt1x_fw_version.sensor_id);
...
int init_chip_type(void)
{
	...
	if (!strncmp((const char *)gt1x_fw_version.product_id, "1143", 4)) {
		sys.chip_type = _GT1143;
	} else if (!strncmp((const char *)gt1x_fw_version.product_id, "1133", 4)) {
		sys.chip_type = _GT1133;
	} else if (!strncmp((const char *)gt1x_fw_version.product_id, "1151", 4)) {
