...
static int yl_params_wait_busy(struct mmc_card *card)
{
	...
	do {
		...
		if (!busy && !(cmd.resp[0] & R1_READY_FOR_DATA)) {
			busy = 1;
			pr_warn("%s: Host didn't wait for busy state to end\n",
...
extern int yl_cmdq_switch(struct mmc_card *card, bool enable);
		...
		sector_t offset, char *buf, int write)
{
	...
	if (mmc_card_cmdq(card)) {
		cmdq_flag = 1;
		err = yl_cmdq_switch(card, false);
		if (err)
			pr_err("mmc can not exit cmdq\n");
		else
			pr_err("mmc cmdq exit OK\n");
	}
	...
	if (1 == cmdq_flag) {
		cmdq_flag = 0;
		err = yl_cmdq_switch(card, true);
		if (err)
			pr_err("mmc0 can not enter cmdq\n");
		else
			pr_err("mmc0 cmdq enter OK\n");
	} else
		pr_err("mmc not in cmdq mode...\n");
...
static ssize_t yl_params_read_locked(char *buf)
{
	...
	if (len == ONE_BLOCK_SIZE)
		...
		pr_err("read block %s failed, ret %zd\n", buf, len);
	return len;
}
		...
		size_t count, loff_t *ppos)
{
	...
	if (count <= TAG_LENGTH)
		pr_debug("%s read count %zd\n", __func__, count);

	mutex_lock(&yl_param_lock);
	count_temp = copy_from_user(kernel_buf, buf,
			count < TAG_LENGTH ? count : TAG_LENGTH);
	if (count_temp != 0)
		pr_info("%s copy from left %zd\n", __func__, count_temp);

	len = yl_params_read_locked(kernel_buf);
	if (len > 0) {
		if (len > count)
			len = count;
		count_temp = copy_to_user(buf + TAG_LENGTH,
				kernel_buf + TAG_LENGTH,
				len > TAG_LENGTH ? len - TAG_LENGTH : 0);
		if (count_temp) {
			pr_info("%s copy to left %zd\n", __func__, count_temp);
			len -= count_temp;
		}
	}
	mutex_unlock(&yl_param_lock);
...
ssize_t show_preload(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	...
	mutex_lock(&yl_param_lock);
	for (i = 0; i < YL_PARAMS_COUNT; i++)
		len += snprintf(buf + len, PAGE_SIZE, "%16s\t%s\n",
				yl_params_map[i],
...
static int yl_reboot(struct notifier_block *nb,
...
int yl_params_init(struct mmc_card *card)
{
	...
	strlcpy(recovery_buf, "boot-recovery", ONE_BLOCK_SIZE);
